...
text \<open>\ken Before we can prove more interesting results on projective planes, we first prove some
      basic facts: \<close>

lemma collinear_order_swaps_dont_matter:
  fixes P Q R
  assumes "collinear P Q R"
  shows "collinear P R Q"
by (metis (mono_tags, lifting) assms projective_plane_data.collinear_def)

lemma collinear_order_rotations_dont_matter:
  fixes P Q R
  assumes "collinear P Q R"
  shows "collinear R P Q "
by (metis (mono_tags, lifting) assms projective_plane_data.collinear_def)

lemma no_distinct_lines_meet_twice:
  fixes P Q l m
  assumes "P \<noteq> Q"
  assumes "meets P l \<and> meets Q l"
  assumes "meets P m \<and> meets Q m"
  shows "l = m"
proof -
  have p2_here: "l \<noteq> m \<Longrightarrow> \<exists>!R. meets R l \<and> meets R m"
    using p2 by auto
  have "\<not> (\<exists>! R. meets R l \<and> meets R m)"
    using assms by auto
  thus ?thesis
    using p2_here by linarith
qed

lemma four_collinear_on_same_line:
  fixes P Q R S
  assumes "P \<noteq> Q \<and> P \<noteq> R \<and> P \<noteq> S \<and> Q \<noteq> R \<and> Q \<noteq> S \<and> R \<noteq> S"
  assumes "collinear P Q R"
  assumes "collinear Q R S"
  shows "\<exists>l. meets P l \<and> meets Q l \<and> meets R l \<and> meets S l"
proof -
  obtain l where l: "meets P l \<and> meets Q l \<and> meets R l"
    using projective_plane_data.collinear_def assms by metis
  obtain m where m: "meets Q m \<and> meets R m \<and> meets S m"
    using projective_plane_data.collinear_def assms by metis
  have "l = m"
    by (meson assms(1) l m projective_plane.no_distinct_lines_meet_twice projective_plane_axioms)
  thus ?thesis
    using l m by auto
qed
...
lemma linesMeetOnce:
  ...
  assumes "collinear P R S"
  shows "meets T l \<and> collinear T R S \<Longrightarrow> T = P"
by (smt assms no_distinct_lines_meet_twice p1 collinear_def)

  text \<open>\ken We can now move on to more interesting results about projective planes that Hartshorne
        gives as an exercise in the book: \<close>
...
theorem all_lines_have_the_same_cardinality:
  fixes l and m
...
proof -
  have l_pt_meet_l: "\<lbrakk>L \<in> l_pts\<rbrakk> \<Longrightarrow> meets L l"
  ...
  next
    ...
    show ?thesis
    proof -
      obtain R where R: "\<not> meets R l \<and> \<not> meets R m"
        using pointOffLines False by auto
      have Q: "\<lbrakk>Q \<in> l_pts\<rbrakk> \<Longrightarrow> meets Q l"
        by (simp add: assms(1))
      have not_equal:"\<lbrakk>Q \<in> l_pts\<rbrakk> \<Longrightarrow> Q \<noteq> R"
        using Q R by auto
      have exists_line: "\<lbrakk>Q \<in> l_pts\<rbrakk> \<Longrightarrow> \<exists>QR. meets Q QR \<and> meets R QR \<and> QR \<noteq> m"
        using R p1 not_equal by blast
      have one_meets: "\<lbrakk>Q \<in> l_pts\<rbrakk> \<Longrightarrow> \<exists>!P.  meets P m \<and> (\<exists>QR. meets Q QR \<and> meets R QR \<and>  meets P QR)"
        using p2 exists_line R Q not_equal p1
        by blast

      have "meets A l \<and> meets B l \<and> (\<exists>n. meets A n \<and> meets B n \<and> meets R n) \<Longrightarrow> A = B"
        using R p1 by blast
      have coll_imp:
        "collinear S R T \<and> collinear U R T \<and> meets S l \<and> meets U l \<and> meets T m \<Longrightarrow> S = U"
        by (smt R no_distinct_lines_meet_twice p1 collinear_def)
      have f_def_inj: "(meets S l \<and> meets T m \<and> (\<exists>n. meets S n \<and> meets R n \<and> meets T n)) \<and>
                       (meets U l \<and> meets T m \<and> (\<exists>n. meets U n \<and> meets R n \<and> meets T n))
                        \<Longrightarrow> S = U"
        using coll_imp collinear_def by auto

      (* Create a bijection from l_pts to m_points to show they are the same size *)
      define f where  f: "f \<equiv> \<lambda>S T. (meets S l \<and> meets T m \<and> (\<exists>n. meets S n \<and> meets R n \<and> meets T n))"
     
      define g :: "'point \<Rightarrow> 'point " where g: "g \<equiv> \<lambda>A. (THE B. f A B)"

      have functional: "f A B \<and> f A C \<Longrightarrow> B = C"
        sorry
       (* this has timeout issues part of the time
        by (smt R \<open>f \<equiv> \<lambda>S T. meets S l \<and> meets T m \<and> (\<exists>n. meets S n \<and> meets R n \<and> meets T n)\<close> no_distinct_lines_meet_twice) 
       *)
       

      have f_inj: "f A C \<and> f B C \<Longrightarrow> A = B"
        (* this proof was generated by sledgehammer *)
      proof -
        assume a1: "f A C \<and> f B C"
        obtain ll :: "'point \<Rightarrow> 'point \<Rightarrow> 'line" where
          f2: "\<forall>x0 x1. (\<exists>v2. meets x1 v2 \<and> meets R v2 \<and> meets x0 v2) = (meets x1 (ll x0 x1) \<and> meets R (ll x0 x1) \<and> meets x0 (ll x0 x1))"
          by moura
        then have "(\<not> f A C \<or> meets A l \<and> meets C m \<and> meets A (ll C A) \<and> meets R (ll C A) \<and> meets C (ll C A)) \<and> (f A C \<or> \<not> meets A l \<or> \<not> meets C m \<or> (\<forall>l. \<not> meets A l \<or> \<not> meets R l \<or> \<not> meets C l))"
          using \<open>f \<equiv> \<lambda>S T. meets S l \<and> meets T m \<and> (\<exists>n. meets S n \<and> meets R n \<and> meets T n)\<close> by blast
        then have f3: "meets A l \<and> meets C m \<and> meets A (ll C A) \<and> meets R (ll C A) \<and> meets C (ll C A)"
          by (simp add: a1)
        then have f4: "\<forall>p pa l la. (p = pa \<or> \<not> meets p l \<or> \<not> meets pa l \<or> \<not> meets p la \<or> \<not> meets pa la) \<or> l = la"
          using no_distinct_lines_meet_twice by blast
        then have f5: "meets B l \<and> meets C m \<and> meets B (ll C B) \<and> meets R (ll C B) \<and> meets C (ll C B)"
          using f2 a1 \<open>f \<equiv> \<lambda>S T. meets S l \<and> meets T m \<and> (\<exists>n. meets S n \<and> meets R n \<and> meets T n)\<close> by auto
        then have "ll C B = ll C A"
          using f4 f3 by (metis (no_types) R)
        then have "meets B (ll C A)"
          using f5 by metis
        thus ?thesis
          using f5 f4 f3 R by blast
      qed


      have right: "\<lbrakk>A \<in> l_pts; B \<in> m_pts\<rbrakk> \<Longrightarrow> (f A B \<longrightarrow> (g A) = B)" 
        unfolding g
        using R f no_distinct_lines_meet_twice theI_unique by smt

      have left: "\<lbrakk>A \<in> l_pts; B \<in> m_pts\<rbrakk> \<Longrightarrow> ((g A) = B \<longrightarrow> f A B)" unfolding g
        sorry

      have same: "\<lbrakk>A \<in> l_pts; B \<in> m_pts\<rbrakk> \<Longrightarrow> (f A B \<longleftrightarrow> (g A) = B)"
        using right left by auto

      have f_sat: "\<lbrakk>Q \<in> l_pts\<rbrakk> \<Longrightarrow> \<exists> P. f Q P" 
        unfolding f 
        using one_meets assms(1) l_pt_meet_l
        by auto 

      have f_surj: "\<lbrakk>M \<in> m_pts\<rbrakk> \<Longrightarrow> \<exists> Q. f Q M"
        sorry
        
      have from_l_to_m: "\<lbrakk>L \<in> l_pts \<rbrakk> \<Longrightarrow> (g L) \<in> m_pts"
        sorry

      have foreach_exists: "\<lbrakk>M \<in> m_pts\<rbrakk> \<Longrightarrow> \<exists> L \<in> l_pts. (g L = M)" unfolding g f sorry

      have surjective: "g ` l_pts = m_pts" 
        using foreach_exists from_l_to_m
        sorry

      have injection: "inj_on g l_pts" unfolding g using f_inj functional f
        sorry

      have bijection: "bij_betw g l_pts m_pts" 
        using surjective injection Fun.bij_betw_imageI by auto
...
(*
    proof (cases "l || m")
      ...
      obtain f where "f S = S"

      fix f
      assume maps_to_one_on_m:
